...
global $wpdb;
if ( empty( $url ) )
//return new WP_Error( 'no_image_url', __( 'No image URL has been entered.','medipress' ), $url );
...
if ( 'bmp' == $ext ) {
return new WP_Error( 'bmp_mime_type', __( 'Image is BMP. Please use either JPG or PNG.','medipress' ), $url );
}
...
if ( !$size )
return new WP_Error( 'file_path_getimagesize_failed', __( 'Failed to get $file_path information using "@getimagesize".','medipress'), $file_path );
...
return new WP_Error( 'resize_path_invalid', __( 'Resize path invalid (GIF)','medipress' ) );
}
elseif ( IMAGETYPE_PNG == $orig_type ) {
if ( !imagepng( $new_image, $dest_file_name ) )
return new WP_Error( 'resize_path_invalid', __( 'Resize path invalid (PNG).','medipress' ) );
}
else {
...
return new WP_Error( 'resize_path_invalid', __( 'Resize path invalid (JPG).','medipress' ) );
}
...
if ( !$new_size )
return new WP_Error( 'resize_path_getimagesize_failed', __( 'Failed to get $dest_file_name (resized image) info via @getimagesize','medipress' ), $dest_file_name );
