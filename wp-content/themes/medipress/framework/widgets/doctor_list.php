...
class RO_Doctor_List_Widget extends RO_Widget {
	public function __construct() {
		...
		$this->widget_description = __( 'Display a list of your doctor on your site.', 'medipress' );
		$this->widget_id          = 'ro_doctor_list';
		$this->widget_name        = __( 'Doctor List', 'medipress' );
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'std'   => __( 'Doctor List', 'medipress' ),
				'label' => __( 'Title', 'medipress' )
			),
			'doctor_department' => array(
				'type'   => 'tb_taxonomy',
				'std'    => '',
				'label'  => __( 'Department', 'medipress' ),
			),
					...
					<li class="ro-doctor-item clearfix">
						<a href="<?php the_permalink(); ?>">
							...
							<div class="ro-content">
								<h6><?php the_title(); ?></h6>
								<span><?php echo __('Major: ', 'medipress').get_post_meta( get_the_ID(), 'tb_doctor_major', true ); ?></span>
							</div>
						</a>
					</li>
				<?php } ?>
...
/* Class RO_Doctor_List_Widget */
function register_doctor_list_widget() {
    register_widget('RO_Doctor_List_Widget');
}
