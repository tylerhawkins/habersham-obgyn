<?php
class mediPress_Recent_Posts_Thumbnails extends WP_Widget {
	...
	function __construct() {
        ...
        parent::__construct(
            ...
            esc_html__('mediPress Recent Posts Thumbnails', 'medipress'), // Name
            array('description' => esc_html__('List of your site&#8217;s most recent posts, with clickable title and thumbnails.', 'medipress'),) // Args
        );
		...
		<?php while ( $r->have_posts() ) : $r->the_post(); ?>
			<li>
					...
					if ( $show_thumb ) : 
						...
						if ( $only_1st_img ) :
							...
							if ( has_post_thumbnail() ) : 
								...
								$image_url = matthewruddy_image_resize( $attachment_image[0], $thumb_width, $thumb_height, true, false );
				...
				<div class="recent-detail">
					...
					<?php if ( $show_date ) : ?> 
						<div class="zo-post-date"><span><?php echo get_the_date('F j, Y');?></span></div>
					<?php	endif; ?>
...
<?php
	...
	function form( $instance ) {
		...
		?>
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title:', 'medipress' ); ?></label>
		<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></p>

		<p><label for="<?php echo esc_attr($this->get_field_id( 'number_posts' )); ?>"><?php esc_html_e( 'Number of posts to show:', 'medipress' ); ?></label>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'show_date' )); ?>"><?php esc_html_e( 'Display post date?', 'medipress' ); ?></label></p>

		<h4><?php esc_html_e( 'Thumbnail Options', 'medipress' ); ?>:</h4>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'show_thumb' )); ?>"><?php esc_html_e( 'Display post featured image?', 'medipress' ); ?></label></p>
		
		<p><label for="<?php echo esc_attr($this->get_field_id( 'thumb_width' )); ?>"><?php esc_html_e( 'Width of thumbnail', 'medipress' ); ?>:</label>
		...
		<p><label for="<?php echo esc_attr($this->get_field_id( 'thumb_height' )); ?>"><?php esc_html_e( 'Height of thumbnail', 'medipress' ); ?>:</label>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'keep_aspect_ratio' )); ?>"><?php esc_html_e( 'Use aspect ratios of original images?', 'medipress' ); ?> <em><?php esc_html_e( 'If checked the given width is used to determine the height of the thumbnail automatically. This option also supports responsive web design.', 'medipress' ); ?></em></label></p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'hide_title' )); ?>"><?php esc_html_e( 'Do not show title?', 'medipress' ); ?> <em><?php esc_html_e( 'Make sure you set a default thumbnail for posts without a thumbnail, otherwise there will be no link.', 'medipress' ); ?></em></label> </p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'try_1st_img' )); ?>"><?php esc_html_e( "Try to use the post's first image if post has no featured image?", 'medipress' ); ?></label></p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'only_1st_img' )); ?>"><?php esc_html_e( 'Use first image only, ignore featured image?', 'medipress' ); ?></label></p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'use_default' )); ?>"><?php esc_html_e( 'Use default thumbnail if no image could be determined?', 'medipress' ); ?></label></p>
		
		<p><label for="<?php echo esc_attr($this->get_field_id( 'default_url' )); ?>"><?php esc_html_e( 'URL of default thumbnail (start with http://)', 'medipress' ); ?>:</label>
 ...
 * @since 1.0
 */
function medipress_register_recent_posts_widget_with_thumbnails () {
	register_widget('mediPress_Recent_Posts_Thumbnails');
}
add_action('widgets_init', 'medipress_register_recent_posts_widget_with_thumbnails');