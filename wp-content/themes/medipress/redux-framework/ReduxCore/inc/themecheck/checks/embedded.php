            ...
            if ( $redux ) {
                if ( ! isset( $_POST['redux_wporg'] ) ) {
                    ...
                    if ( file_exists( $tracking ) ) {
                        $this->error[] = '<div class="redux-error">' . sprintf( __('<span class="tc-lead tc-required">REQUIRED</span>: You MUST delete <strong> %s </strong>, or your theme will be rejected by WP.org theme submission because of Redux.', 'medipress'), $tracking ) . '</div>';
                        $ret           = false;
                    }
                    ...
                    $args .= "<li><code>'database' => 'theme_mods'</code> (" . __( 'Optional', 'medipress' ) . ")</li>";
                    $args .= '</ol>';
                    $this->error[] = '<div class="redux-error">' . __( '<span class="tc-lead tc-recommended">RECOMMENDED</span>: The following arguments MUST be used for WP.org submissions, or you will be rejected because of your Redux configuration.', 'medipress' ) . $args . '</div>';
