    ...
    if ( ! class_exists( 'reduxCorePanel' ) ) {
        ...
        class reduxCorePanel {
            ...
            private function panel_template() {
                ...
                // Do we support JS?
                echo '<noscript><div class="no-js">' . __( 'Warning- This options panel will not work properly without javascript!', 'medipress' ) . '</div></noscript>';
            ...
            function notification_bar() {
                ...
                if ( isset( $this->parent->transients['last_save_mode'] ) ) {
                    ...
                    if ( $this->parent->transients['last_save_mode'] == "import" ) {
                         ...
                         */
                        echo '<div class="admin-notice notice-blue saved_notice"><strong>' . apply_filters( "redux-imported-text-{$this->parent->args['opt_name']}", __( 'Settings Imported!', 'medipress' ) ) . '</strong></div>';
                        //exit();
                         ...
                         * @param string  translated "settings imported" text
                         */
                        echo '<div class="saved_notice admin-notice notice-yellow"><strong>' . apply_filters( "redux-defaults-text-{$this->parent->args['opt_name']}", __( 'All Defaults Restored!', 'medipress' ) ) . '</strong></div>';
                         ...
                         * @param string  translated "settings imported" text
                         */
                        echo '<div class="saved_notice admin-notice notice-yellow"><strong>' . apply_filters( "redux-defaults-section-text-{$this->parent->args['opt_name']}", __( 'Section Defaults Restored!', 'medipress' ) ) . '</strong></div>';
                         ...
                         */
                        echo '<div class="saved_notice admin-notice notice-green">' . apply_filters( "redux-saved-text-{$this->parent->args['opt_name']}", '<strong>'.__( 'Settings Saved!', 'medipress' ) ).'</strong>' . '</div>';
                    }
                 ...
                 */
                echo '<div class="redux-save-warn notice-yellow"><strong>' . apply_filters( "redux-changed-text-{$this->parent->args['opt_name']}", __( 'Settings have changed, you should save them!', 'medipress' ) ) . '</strong></div>';
                ...
                echo '<div class="redux-field-errors notice-red"><strong><span></span> ' . __( 'error(s) were found!', 'medipress' ) . '</strong></div>';
                ...
                echo '<div class="redux-field-warnings notice-yellow"><strong><span></span> ' . __( 'warning(s) were found!', 'medipress' ) . '</strong></div>';
                    ...
                    if ( $developer_theme_file ) {
                        ...
                        if ( $core_version && $developer_version && version_compare( $developer_version, $core_version, '<' ) ) {
                            ...
                            <div id="message" class="error redux-message">
                                <p><?php _e( '<strong>Your panel has bundled outdated copies of Redux Framework template files</strong> &#8211; if you encounter functionality issues this could be the reason. Ensure you update or remove them.', 'medipress' ); ?></p>
                            </div>
                            <?php
                            return;
                        }
