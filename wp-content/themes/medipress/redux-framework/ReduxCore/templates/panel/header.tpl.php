<?php
    ...
    $tip_title = __( 'Developer Mode Enabled', 'medipress' );
    ...
    if ( $this->parent->dev_mode_forced ) {
        ...
        if ( Redux_Helpers::isWpDebug() ) {
            $is_debug  = true;
            $debug_bit = __( 'WP_DEBUG is enabled', 'medipress' );
        }

        $localhost_bit = '';
        if ( Redux_Helpers::isLocalHost() ) {
            $is_localhost  = true;
            $localhost_bit = __( 'you are working in a localhost environment', 'medipress' );
        }

        $conjunction_bit = '';
        if ( $is_localhost && $is_debug ) {
            $conjunction_bit = ' ' . __( 'and', 'medipress' ) . ' ';
        }

        $tip_msg = __( 'This has been automatically enabled because', 'medipress' ) . ' ' . $debug_bit . $conjunction_bit . $localhost_bit . '.';
    } else {
        $tip_msg = __( 'If you are not a developer, your theme/plugin author shipped with developer mode enabled. Contact them directly to fix it.', 'medipress' );
    }
        ...
        <div class="display_header">
            ...
            <?php if ( isset( $this->parent->args['dev_mode'] ) && $this->parent->args['dev_mode'] ) { ?>
                <div class="redux-dev-mode-notice-container redux-dev-qtip"
                    ...
                    <span
                        class="redux-dev-mode-notice"><?php _e( 'Developer Mode Enabled', 'medipress' ); ?></span>
                </div>
            <?php } elseif (isset($this->parent->args['forced_dev_mode_off']) && $this->parent->args['forced_dev_mode_off'] == true ) { ?>
                ...
                <div class="redux-dev-mode-notice-container redux-dev-qtip" 
                     ...
                     qtip-content="<?php echo esc_attr( $tip_msg ); ?>">
                    <span
                        class="redux-dev-mode-notice" style="background-color: #FF001D;"><?php _e( 'FORCED DEV MODE OFF ENABLED', 'medipress' ); ?></span>
                </div>
